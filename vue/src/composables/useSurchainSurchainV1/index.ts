...
export default function useSurchainSurchainV_1() {
  const client = useClient();

  type QueryParamsMethod = typeof client.SurchainSurchainV_1.query.queryParams;
  ...
  const QueryParams = ( options: Partial<UseQueryOptions<QueryParamsData>>) => {
    ...
    return useQuery<QueryParamsData>({ queryKey: [key], queryFn: async () => {
      const res = await client.SurchainSurchainV_1.query.queryParams();
        return res.data;
    }, ...options});
  }
